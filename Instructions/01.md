...
1. Copy the below URL and paste it in a new tab on the Edge browser inside your JumpHost where you have already logged in to the Azure portal. If the credentials window appears again, then enter the Azure credentials provided below.
...
1. On the **Platform management, security, and governance** blade, you will configure the core components to enable platform monitoring and security. The options you enable will also be enforced using Azure Policy to ensure resources, landing zones, etc are continuously compliant as your deployments scale and grows. To enable this, you must provide a dedicated (empty) subscription that will be used to host the requisite infrastructure. Use the following configuration:
...
1. Under the **Platform DevOps and automation** section, make sure `Deploy integrated CICD pipeline` is set to **Yes (recommended)**, also verify the below information.
   ...
   - **New GitHub repository name (2):** `eslz`, the repository will be created with this name
   - **GitHub personal access token (3):** Use the token which you have generated in the previous step
...
1. On the **Register an application** blade, enter following details:
   - Name: `eslzsp`
...
1. After registering, the application blade will open up automatically. Under the **Certificates and secrets (1)** section, click on **+ New Client secret (2).**   
